...
Moved the package doc comment out of `media.go` into a dedicated `doc.go` — the file pkg.go.dev actually looks for. Mechanical relocation only, content unchanged (already accurate). No behaviour change.
